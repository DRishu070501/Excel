...
def create_dynamic_grid(data):
    ...
    # Add the new grid
    grid = [[tk.Entry(canvas_frame, width=10, font=("Arial", 10)) for c in range(cols)] for r in range(rows)]
...
buttons = [
    ("Load File (Excel/CSV)", load_file),
    ("Load from Google Sheets", load_from_google_sheets),
    ("Save to Google Sheets", save_to_google_sheets),
